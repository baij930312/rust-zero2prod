...
pub async fn subscribe(
  ...
  if  store_token( &mut transaction,subscriber_id,&subscription_token)   .await   
    ...
    if send_confirmation_email(new_subscriber, &email_client,&base_url.0,&subscription_token)
        .await
        .is_err() 
    {
        return HttpResponse::InternalServerError().finish();
...
pub async fn send_confirmation_email(
    ...
    let confrimation_link =format!( "{}/subscriptions/confirm?subscription_token={}",base_url,subscription_token);
...
#[tracing::instrument(name = "Saving as a new  subscriber", skip(new_subscriber, transaction))]
...
#[tracing::instrument(name = "Store subscription token in the db", skip(subscription_token, transaction))]
